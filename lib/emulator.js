...
function getSaveStorageKey(checksum = currentRomChecksum) {
    return checksum ? `jsboy_save_${checksum}` : null;
}

function getRtcStorageKey(checksum = currentRomChecksum) {
    return checksum ? `jsboy_rtc_${checksum}` : null;
}

function encodeRamData(ramData) {
    let binary = '';
    const chunkSize = 0x8000;
    for (let i = 0; i < ramData.length; i += chunkSize) {
        const chunk = ramData.subarray(i, i + chunkSize);
...
function loadRtcState() {
    const rtcStorageKey = getRtcStorageKey();
    if (!rtcStorageKey) {
        return null;
    }

    const rawRtcState = localStorage.getItem(rtcStorageKey);
    if (!rawRtcState) {
        return null;
    }
...
function persistRtcState(state) {
    persistRtcStateForKey(getRtcStorageKey(), state);
}

function persistRtcStateForKey(rtcStorageKey, state) {
    if (!rtcStorageKey) {
        return;
    }

    localStorage.setItem(rtcStorageKey, JSON.stringify(state));
}
    ...
    return cgbFlag === 0x80 || cgbFlag === 0xC0 ? 'CGB' : 'DMG';
        ...
        class MBC1 {
            constructor(romData, savedRam = null) {
                ...
                this.saveKey = getSaveStorageKey();
            ...
            writeByte(address, value) {
             ...
             requestSave() {
                ...
                this.saveDebounceTimer = setTimeout(() => {
                    const saveData = encodeRamData(this.ram);
        ...
        class MBC3 {
            ...
            requestSave() {
                ...
                this.saveDebounceTimer = setTimeout(() => {
                    const saveData = encodeRamData(this.ram);
                    localStorage.setItem(this.saveKey, saveData);
                    persistRtcStateForKey(this.rtcKey, this._getRtcState());
                    emitSaveStatus("Spielstand gespeichert.", 2000);
                }, 1500);
            }
        }
        ...
        class MMU {
            constructor(gameboy) {
                ...
                this.objPaletteData = new Uint8Array(0x40);
                this.vramBank = 0;
                this.wramBank = 1;
                this.hdmaSource = 0;
            ...
            readByte(address) {
                ...
                if (address === 0xFF6A) {
                    return 0x40 | (this.objPaletteAutoIncrement ? 0x80 : 0x00) | (this.objPaletteIndex & 0x3F);
                }
                if (address === 0xFF6B) {
                    return this.readObjPaletteByte();
                }
                ...
                if (address === 0xFF55) {
                    ...
                    return (this.hdmaActive ? 0x00 : 0x80) | (this.hdmaLength & 0x7F);
                }
                if (address === 0xFF70) {
            ...
            writeByte(address, value) {
                ...
                if (address >= 0xFF51 && address <= 0xFF54) {
                    this.writeHdmaRegister(address, value);
                    return;
                }
                if (address === 0xFF55) {
                    this.writeHdmaControl(value);
                    return;
                }
                if (address === 0xFF01) {
                ...
                if (address === 0xFF6A) {
                    ...
                    this.memory[address] = 0x40 | (this.objPaletteAutoIncrement ? 0x80 : 0x00) | (this.objPaletteIndex & 0x3F);
                    return;
                }
                if (address === 0xFF6B) {
                    this.writeObjPaletteByte(value);
                    return;
                }
                if (address >= 0x8000 && address <= 0x9FFF) {
                    if (this.isPpuMode(3)) {
            ...
            readVramBankByte(address, bank = 0) {
                return this.vramBanks[bank & 0x01][address - 0x8000];
            }
            ...
            writeVramBankByte(address, bank, value) {
                this.vramBanks[bank & 0x01][address - 0x8000] = value;
            }
            ...
            writeObjPaletteByte(value) {
                ...
                if (this.objPaletteAutoIncrement) {
                    ...
                    this.memory[0xFF6A] = 0x40 | 0x80 | this.objPaletteIndex;
                }
            }

            initializeIoDefaults(hardwareMode) {
                const defaults = {
                    0xFF00: 0xCF,
                    0xFF05: 0x00,
                    0xFF06: 0x00,
                ...
                Object.entries(defaults).forEach(([address, value]) => {
                    this.memory[Number(address)] = value;
                });

                this.memory[0xFF4D] = hardwareMode === 'CGB' ? 0x00 : 0xFF;
                this.memory[0xFF4F] = 0xFE;
                this.memory[0xFF51] = 0x00;
                this.memory[0xFF52] = 0x00;
                this.memory[0xFF53] = 0x00;
                this.memory[0xFF54] = 0x00;
                this.memory[0xFF55] = 0xFF;
                this.memory[0xFF68] = 0x40;
                this.memory[0xFF6A] = 0x40;
            ...
            getKey1Value() {
                ...
                return (this.gameboy.doubleSpeed ? 0x80 : 0x00) | (this.gameboy.prepareSpeedSwitch ? 0x01 : 0x00);
            }
            ...
            writeKey1(value) {
                ...
                this.gameboy.prepareSpeedSwitch = (value & 0x01) !== 0;
                this.memory[0xFF4D] = this.getKey1Value();
            }

            writeHdmaRegister(address, value) {
                if (!this.gameboy || this.gameboy.hardwareMode !== 'CGB') {
                    return;
                }
                ...
                switch (address) {
                    case 0xFF51:
                        this.hdmaSource = ((value << 8) | (this.hdmaSource & 0x00FF)) & 0xFFF0;
                        this.memory[address] = value;
                        break;
                    case 0xFF52:
                        this.hdmaSource = ((this.hdmaSource & 0xFF00) | (value & 0xF0)) & 0xFFF0;
                        this.memory[address] = value & 0xF0;
                        break;
                    case 0xFF53:
                        this.hdmaDestination = 0x8000 | (((value & 0x1F) << 8) | (this.hdmaDestination & 0x00FF));
                        this.hdmaDestination &= 0x9FF0;
                        this.memory[address] = value & 0x1F;
                        break;
                    case 0xFF54:
                        this.hdmaDestination = 0x8000 | ((this.hdmaDestination & 0x1F00) | (value & 0xF0));
                        this.hdmaDestination &= 0x9FF0;
                        this.memory[address] = value & 0xF0;
                        break;
                }
            ...
            writeHdmaControl(value) {
                ...
                if (this.hdmaActive && (value & 0x80) === 0) {
                    this.hdmaActive = false;
                    this.memory[0xFF55] = 0x80 | (this.hdmaLength & 0x7F);
                    return;
                }
                ...
                if ((value & 0x80) !== 0) {
                    ...
                    this.memory[0xFF55] = this.hdmaLength & 0x7F;
                    return;
                }
            ...
            transferHdmaBlock() {
                ...
                const destination = 0x8000 | (this.hdmaDestination & 0x1FF0);
                const destinationBank = this.getActiveVramBank();

                for (let i = 0; i < 0x10; i++) {
                    this.writeVramBankByte(destination + i, destinationBank, this.readByte(source + i));
                }

                this.hdmaSource = (source + 0x10) & 0xFFF0;
                this.hdmaDestination = 0x8000 | ((destination + 0x10) & 0x1FF0);
                ...
                if (this.hdmaLength === 0) {
                    ...
                    this.hdmaLength = (this.hdmaLength - 1) & 0x7F;
                    this.memory[0xFF55] = (this.hdmaActive ? 0x00 : 0x80) | this.hdmaLength;
                }

                this.memory[0xFF51] = (this.hdmaSource >> 8) & 0xFF;
                this.memory[0xFF52] = this.hdmaSource & 0xF0;
                this.memory[0xFF53] = (this.hdmaDestination >> 8) & 0x1F;
                this.memory[0xFF54] = this.hdmaDestination & 0xF0;
                return true;
            }
            ...
            loadRom(romData, savedRam, savedRtc = null, hardwareMode = 'DMG') {
                ...
                this.objPaletteAutoIncrement = false;
                if (this.gameboy) {
                    this.gameboy.doubleSpeed = false;
                    this.gameboy.prepareSpeedSwitch = false;
                }
                ...
                this.initializeIoDefaults(hardwareMode);
        ...
        class Timer {
            ...
            reset() {
                this.div = this.mmu.memory[0xFF04] || 0;
                this.tima = this.mmu.memory[0xFF05] || 0;
                this.tma = this.mmu.memory[0xFF06] || 0;
                this.tac = this.mmu.memory[0xFF07] || 0;
                this.divClock = 0;
                this.timaClock = 0;
                this.timaReloadDelay = 0;
            }
        ...
        class PPU {
            constructor(mmu, screenCtx) {
                ...
                this.bgPriorityBuffer = new Uint8Array(this.SCREEN_WIDTH);
            ...
            reset() {
                ...
                this.bgPriorityBuffer.fill(0);
                this.screenData.data.fill(0);
                this.mmu.memory[0xFF44] = 0;
            }

            isCgbMode() {
                return this.mmu.gameboy && this.mmu.gameboy.hardwareMode === 'CGB';
            }

            getBackgroundTileAttributes(tileMapSelect, tileMapIndex) {
                if (!this.isCgbMode()) {
                    return {
                        paletteNumber: 0,
                        vramBank: 0,
                ...
                const attributes = this.mmu.readVramBankByte(tileMapSelect + tileMapIndex, 1);
                return {
                    paletteNumber: attributes & 0x07,
                    vramBank: (attributes >> 3) & 0x01,
            ...
            readBackgroundTileId(tileMapSelect, tileMapIndex) {
                if (this.isCgbMode()) {
                    return this.mmu.readVramBankByte(tileMapSelect + tileMapIndex, 0);
                }
                return this.mmu.readByte(tileMapSelect + tileMapIndex);
            }

            readTilePixelBytes(tileAddress, vramBank) {
                if (!this.isCgbMode()) {
                    return {
                        byte1: this.mmu.readByte(tileAddress),
                        byte2: this.mmu.readByte(tileAddress + 1),
                    };
                }

                return {
                    byte1: this.mmu.readVramBankByte(tileAddress, vramBank & 0x01),
                    byte2: this.mmu.readVramBankByte(tileAddress + 1, vramBank & 0x01),
                };
            }

            decodeCgbColor(lowByte, highByte) {
                const colorValue = lowByte | (highByte << 8);
                const red = colorValue & 0x1F;
                const green = (colorValue >> 5) & 0x1F;
                const blue = (colorValue >> 10) & 0x1F;
            ...
            getCgbPaletteColor(paletteData, paletteNumber, colorId) {
                const colorOffset = ((paletteNumber & 0x07) * 8) + ((colorId & 0x03) * 2);
                return this.decodeCgbColor(
                    paletteData[colorOffset],
                    paletteData[colorOffset + 1],
                );
            }

            getBackgroundColor(colorId, tileAttributes, bgp) {
                if (this.isCgbMode()) {
                    return this.getCgbPaletteColor(
                        this.mmu.bgPaletteData,
                        tileAttributes.paletteNumber,
                        colorId,
                    );
                }
            ...
            getSpriteColor(colorId, attributes, paletteReg) {
                if (this.isCgbMode()) {
                    return this.getCgbPaletteColor(
                        this.mmu.objPaletteData,
                        attributes & 0x07,
                        colorId,
                    );
                }
            ...
            renderBackground(lcdc) {
                ...
                for (let x = 0; x < this.SCREEN_WIDTH; x++) {
                    ...
                    let tileId = this.readBackgroundTileId(tileMapSelect, tileMapIndex);
                    const tileAttributes = this.getBackgroundTileAttributes(tileMapSelect, tileMapIndex);
                    const pixelTileY = tileAttributes.yFlip ? 7 - tileY : tileY;
                    const pixelBitIndex = tileAttributes.xFlip ? tileX : 7 - tileX;

                    let tileAddress;
                    if (tileDataSelect === 0x8000) {
                        tileAddress = 0x8000 + (tileId * 16) + (pixelTileY * 2);
                    }
                    else {
                        ...
                        tileAddress = 0x9000 + (signedTileId * 16) + (pixelTileY * 2);
                    }

                    const { byte1, byte2 } = this.readTilePixelBytes(tileAddress, tileAttributes.vramBank);

                    const bit1 = (byte1 >> pixelBitIndex) & 1;
                    const bit2 = (byte2 >> pixelBitIndex) & 1;
                    const colorId = (bit2 << 1) | bit1;

                    this.bgLineBuffer[x] = colorId;
                    this.bgPriorityBuffer[x] = tileAttributes.bgPriority ? 1 : 0;

                    const color = this.getBackgroundColor(colorId, tileAttributes, bgp);
            ...
            renderSprites(lcdc) {
                ...
                for (const sprite of visibleSprites) {
                    ...
                    const vramBank = this.isCgbMode() ? ((attributes >> 3) & 0x01) : 0;
                    ...
                    const { byte1, byte2 } = this.readTilePixelBytes(tileAddress, vramBank);

                    for (let x = 0; x < 8; x++) {
                        const screenX = sprite.xPos + x;
                        if (screenX >= 0 && screenX < this.SCREEN_WIDTH) {
                            const bgHasColor = this.bgLineBuffer[screenX] !== 0;
                            const cgbBgPriority = this.isCgbMode() && this.bgPriorityBuffer[screenX] !== 0 && bgHasColor;

                            if (cgbBgPriority) {
                                continue;
                            }

                            if (bgPriority && bgHasColor) {
                                continue;
                            }
                            ...
                            const color = this.getSpriteColor(colorId, attributes, paletteReg);
        ...
        class CPU {
            ...
            executeOpcode(opcode) {
                ...
                switch(opcode) {
                    ...
                    case 0x10:
                        this.pc++;
                        if (!(this.mmu.gameboy && this.mmu.gameboy.toggleDoubleSpeed())) {
                            this.halted = true;
                        }
                        break;
        ...
        export class GameBoy {
            ...
            toggleDoubleSpeed() {
                if (this.hardwareMode !== 'CGB' || !this.prepareSpeedSwitch) {
                    return false;
                }

                this.doubleSpeed = !this.doubleSpeed;
                this.prepareSpeedSwitch = false;
                this.mmu.memory[0xFF4D] = this.mmu.getKey1Value();
                return true;
            }
            ...
            loadRom(romData) {
                ...
                this.cpu.reset(this.hardwareMode);
                this.timer.reset();
                this.ppu.reset();
            ...
            frame() {
                ...
                while(cycles < cyclesPerFrame) {
                    ...
                    const componentCycles = this.doubleSpeed ? executedCycles / 2 : executedCycles;
                    this.mmu.step(componentCycles);
                    this.ppu.step(componentCycles);
                    this.timer.step(componentCycles);
                    this.apu.step(componentCycles);
