    ...
    return currentRomChecksum;
        ...
        class MMU {
            ...
            dmaTransfer(value) {
                ...
                for (let i = 0; i < 0xA0; i++) {
                    this.writeByte(0xFE00 + i, this.readByte(startAddress + i));
                }
            }

            // Corrected loadRom method in MMU, which accepts savedRam
            loadRom(romData, savedRam) {
                ...
                switch (cartridgeType) {
                    ...
                    case 0x0F: case 0x10: case 0x11: case 0x12: case 0x13:
                        ...
                        this.mbc = new MBC3(romData, savedRam);
                        break;
        ...
        class Timer {
            ...
            step(cycles) {
                ...
                while (this.timaClock >= freq) {
                    this.timaClock -= freq;
                    this.tima = this.mmu.readByte(0xFF05);
                    if (this.tima === 0xFF) {
                        this.tima = this.mmu.readByte(0xFF06);
                        let ifReg = this.mmu.readByte(0xFF0F);
                        this.mmu.writeByte(0xFF0F, ifReg | 0x04);
                    } else {
                        this.tima++;
                    }
                    this.mmu.writeByte(0xFF05, this.tima);
                }
            }
        ...
        class PPU {
            ...
            step(cycles) {
                ...
                this.mmu.writeByte(0xFF44, this.line);
            }
        ...
        class CPU {
            ...
            executeOpcode(opcode) {
                ...
                switch(opcode) {
                    ...
                    case 0xF8: { const offset = this.mmu.readByte(this.pc); this.pc++; const s = offset << 24 >> 24; this.hl = (this.sp + s) & 0xFFFF; this.f = 0; if (((this.sp & 0x0F) + (s & 0x0F)) > 0x0F) this.f |= this.FLAG_H; if (((this.sp & 0xFF) + (s & 0xFF)) > 0xFF) this.f |= this.FLAG_C; cycles = 12; break; }
                    ...
                    case 0x00: break; case 0xF3: this.ime = false; break; case 0xFB: this.ime = true; break;
        ...
        export class GameBoy {
            ...
            loadRom(romData) {
                ...
                this.mmu.loadRom(romData, savedRam);
